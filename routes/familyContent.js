...
router.post(
    ...
    async (req, res) => {
      try {
        ...
        const { familyId, contentType, title, description, voiceDuration, metadata } = req.body;
...
router.get("/family/:familyId/:type", protect, async (req, res) => {
  try {
    ...
    const enriched = contents.map((item) => ({
      ...item.toObject(),
      isOwner: item.creator?._id.toString() === req.user._id.toString(),
    }));

    res.json({ success: true, contents: enriched });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
